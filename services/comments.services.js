...
class CommentService {
  ...
  getComment = async (nickname) => {
    const findAllComment = await this.CommentRepository.getComment({
      nickname,
    });
  ...
  postComment = async ({
    ...
    const newComment = await this.CommentRepository.postComment({
      UserId,
      postId,
      nickname,
      comment,
  ...
  putComment = async ({ commentId, UserId, comment }) => {
    ...
    const updateComment = await this.CommentRepository.putComment(
      commentId,
      comment
    );
